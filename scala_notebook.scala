...
//有一个包含 complex 列的 DataFrame，我们可以像查询另一个DataFrame一样查询它，
//唯一的区别是使用“.”来访问或列方法 getField 来实现
...
//用户自定义的函数需要注册后，才能在所有的机器上使用
//需要注册该函数，以便其可用作DataFrame函数
...
//Spark 既支持统计样本标准差也支持统计总体标准差
//它们两个在统计学上是完全不同的概念，一定要区分它们
//如果使用variance 函数和 stddev 函数，默认是计算样本标准差或样本方差的
import org.apache.spark.sql.functions.{var_pop, stddev_pop}
import org.apache.spark.sql.functions.{var_samp, stddev_samp}
df.select(var_pop("Quantity"), var_samp("Quantity"), stddev_pop("Quantity"), stddev_samp("Quantity")).show()

//偏度系数(skewness)和峰度系数(kurtosis)都是对数据集中的极端数据点的衡量指标。偏度
//系数衡量数据相对于平均值的不对称程度，而峰度系数衡量数据分布形态陡缓程度。在将数据
//建模为随机变量的概率分布时，它们都很重要。
import org.apache.spark.sql.functions.{skewness, kurtosis} 
df.select(skewness("Quantity"), kurtosis("Quantity")).show()

//cov和corr，它们分别用于计算协方差和相关性。相关性采用 Pearson 相关系数来衡量，范
//围是-1到+1。协方差的范围由数据中的输入决定。

//聚合输出复杂类型
//在Spark中，不仅可以在数值型上执行聚合操作，还能在复杂类型上执行聚合操作。例如，可
//以收集某列上的值到一个list列表里，或者将unique唯一值收集到一个set集合里。
...
//用户自定义聚合函数（UDAF）：用户定义的聚合函数（UDAF）是用户根据自定义公式或业务逻辑定义自己的聚合函数的一种方法。
//可以使用UDAF来计算输入数据组(与单行相对)的自定义计算。
//Spark维护单个AggregationBuffer，它用于存储每组输入数据的中间结果
...
//左外连接:检查两个DataFrame或表中的键，并包括左侧DataFrame中的所有行以及右
//侧DataFrame中与左侧DataFrame有匹配项的行。如果在右侧DataFrame中没有对应的行，则 Spark 将
//插入null
...
//左半连接:与其他连接操作有点区别，它实际上并不包括右侧DataFrame中的任何值，它只是
//查看左侧DataFrame的值是否存在于右侧DataFrame里，如果存在则在连接结果中保留，即使左侧
//DataFrame中存在重复键，对应行也将保留在结果中。可以将左半连接（Left Semi Join）看作
//DataFrame上的过滤器，而不是常规连接函数.
...
/////////////////数据源
//数据读取：Spark数据读取使用DataFrameReader，通过SparkSession的read属性得到
//spark.read还需要指定以下几个值：
  //format
  //schema
  //read模式
  //一系列option选项
...
//将使用此格式向所有数据源写入数据。format 是可选的，默认情况下Spark将使用Parquet格式，
//option仍用于配置写出数据的方法，PartitionBy，bucketBy和sortBy仅适用于基于文件的数据源，你可
//以使用这些方法来控制写出目标文件的具体结构。
...
val myManualSchema = new StructType(
  Array(
    ...
    nre StructField("ORIGIN_COUNTRY_NAME", StringType, true),
...
//数据分桶
//数据分桶是另一种文件组织方法，你可以使用该方法控制写入每个文件的数据。具有相同桶ID（哈希
//分桶的ID）的数据将放置到一个物理分区中，这样就可以避免在稍后读取数据时进行shuffle（洗牌）。
//根据你之后希望如何使用该数据来对数据进行预分区，就可以避免连接或聚合操作时执行代价
//很大的shuffle操作。
...
//  4、易于使用和操作
//这些特点对于数据分析很有用，因为case类很容易解析。而最重要的特性应该是case类的不变性，
//可以通过内容结构而不是引用进行比较。

/////////////////Scala文档描述：
// 不变性是你无需跟踪对象变化，时间和位置
// 按值比较允许直接比较实例的值，就像他们是基本类型值（primitive value）一样，
// 这样就避免了混淆类实例基于值比较或是基于引用比较所带来的的不确定性.
// 模式匹配简化了分支逻辑，从而会引起更少的bug和带来可读性更好的代码
...
//Transform操作：Dataset的转换操作和DataFrame操作是相同的。Dataset还允许我们指定比DataFrame转换更复杂的和强类型的转换操作，
//因为我们可以操作底层的Java虚拟机(JVM)类型。
  //过滤：
...
//Join
//Dataset连接操作和DataFrame一样，但也提供了更复杂的方法joinWith。 joinWith大致等同于一个co-group(RDD术语)，
//将两个Dataset整合成一个，每一列都表示一个Dataset，并可以相应地进行操作
//当你需要在连接操作中维护更多信息，或执行一些更复杂的操作(如高级映射或筛选)时，这可能很有用。
...
flights2.take(5)
//输出包含一个键值对的Dataset，其中每一行表示一个Flight和Flight元数据
//（Metadata）。当然，我们可以查询这些Dataset或具有复杂类型的DataFrame ：
...
//虽然这提供了灵活性，但需要权衡，因为现在我们引入了JVM类型以及无法被Spark优化的函数。
//这意味着会产生性能差异，当我们检查解释计划时我们可以看到这一点。
//下面你会看到我们正在高效地追加一个新列到DataFrame(分组函数的结果)，然后在该列上执行分组
      ...
      +- *(2) Project [value#118]
         +- AppendColumns <function1>, newInstance(class $line14.$read$$iw$$iw$Flight), 
            ...
            +- *(1) FileScan parquet [DEST_COUNTRY_NAME#0,ORIGIN_COUNTRY_NAME#1,count#2L] 
             Batched: true, 
             Format: Parquet, 
             Location: InMemoryFileIndex[file:/usr/local/spark/testCode/2010-summary.parquet], 
             PartitionFilters: [], 
             PushedFilters: [], 
...
//RDD与DataFrame不同，DataFrame中每个记录即是一个结构化的数据行，
//各字段已知且schema已知,而RDD中的记录仅仅是程序员选择的Java、Scala或Python对象。
//由于只是Java，Python对象，你能完全控制RDD，但也带来很多问题，优化，定义都需要手动执行。
//RDD API与Dataset类似，所不同的是 RDD 不在结构化数据引擎上进行存储或处理。
//然而,在RDD和Dataset之间来回转换的代价很小，因此可以同时使用两种API来取长补短。

//RDD类型：通用型RDD和提供附加函数的key-value RDD
//RDD执行两种（算子）操作：惰性执行的转换（Transform）操作和立即执行的动作（Action）操作，都以分布式的方式来处理数据。
//RDD没有行的概念，单个记录只是简单的Java/Python/Scala对象，必须手动处理这些数据，不能使用结构化API中的函数库。

//创建RDD，只需要使用rdd方法。如果从Dataset[T]转换成RDD，会返回合适的本机类型T（这仅适用于Scala和Java）
spark.range(10).rdd

//要对这些数据进行操作，需要将此Row类型的对象转换为正确的数据类型或从中取出值
spark.range(10).toDF().rdd.map(rowObject => rowObject.getLong(0)) //就变成了Row类型的RDD
...
//-----------------------------------------------------操作RDD：Transformations
//大多数情况下，转换操作（transformation）与结构化API中的转换操作在功能上相同。
//就像使用DataFrame和Dataset一样，需要在RDD上指定转换操作来创建一个新的RDD，
//这也将导致新的RDD依赖于原有的RDD。

//1-----distinct
words.distinct().count()

//2-----filter 等同于创建SQL中的where条件
def startsWithS(individual: String) = {
  individual.startsWith("S")
}

words.filter(word => startsWithS(word)).collect()
//返回：Array[String] = Array(Spark, Sample)
//可以看到，与DatasetAPI一样，它返回的是本地数据类型。
//这是因为我们从不将数据强制转换为Row类型，也不在收集数据后做数据转换操作。
...
//6-----randomSplit : 将一个RDD随机分割为若干个RDD
...
//3-----countApprox 设置置信度，返回大概的计数结果
val confidence = 0.95
val timeoutMilliseconds = 400
word.countApprox(timeoutMilliseconds, confidence)

//4-----countApproxDistinct 计算去重之后大概的个数
words.countApproxDistinct(0.5)

//5-----countByValue 针对值的个数进行计数
words.countByValue()

//6-----countByValueApprox 与上面功能一致，返回近似值
...
//2-----SequenceFiles：一种由二进制键值对组成的文件，它也是Hadoop MapReduce作业常用的输入/输出格式
words.saveAsObjectFile("/tmp/my/sequenceFilePath")


//3-----缓存:缓存RDD的原理与缓存DataFrame和Dataset 相同，你可以缓存或持久化 RDD 。
words.cache()
...
keyword.collect()
res: Array[(String, String)] = Array((s,Spark), (t,The), (d,Definitive), (g,Guide), (:,:), 
                                     (b,Big), (d,Data), (p,Processing), (m,Made), (s,Sample))
...
sampleMap: scala.collection.immutable.Map[Char,Double] = Map(e -> 0.0990014447982912, 
                                                             s -> 0.7882338963599441, 
                                                             n -> 0.13378535260500546, 
                                                             t -> 0.408841397647289, 
                                                             u -> 0.2577941609963563, 
                                                             f -> 0.43970604048491135, 
                                                             a -> 0.6219460901237468, 
                                                             m -> 0.44951842614799986, 
                                                             i -> 0.47327015981663867, 
                                                             v -> 0.9601719414193667, 
                                                             b -> 0.40652491224686127, 
                                                             g -> 0.7179986092554698, 
                                                             l -> 0.7936523752872142, 
                                                             p -> 0.41150247710073784, 
                                                             c -> 0.365471973165296, 
                                                             h -> 0.30438666949669646, 
                                                             r -> 0.245415945051202, 
                                                             : -> 0.9105949719001257, 
                                                             k -> 0.21408162244560203, 
                                                             o -> 0.3235687195940211, 
                                                             d -> 0.012299000197014687)
...
res25: Array[(Char, String)] = Array((s,Spark), (t,The), (d,Definitive), (d,Definitive), (g,Guide), (:,:), 
                                     (b,Big), (p,Processing), (m,Made), (s,Sample))
...
res34: Array[(Char, Int)] = Array((d,4), (p,3), (t,3), (b,1), (h,1), (n,2), (f,1), (v,1), (:,1), (r,2), 
                                  (l,1), (s,4), (e,7), (a,5), (k,1), (i,6), (u,1), (o,1), (g,3), (m,2), (c,1))

//自定义分区是使用RDD的主要原因之一，而结构化API不支持自定义数据分区，RDD包含影响任务能否成功运行的低级实现细节。
//自定义分区的典型示例是PageRank实现，你需要控制集群上数据的分布并避免shuffle操作
//简而言之，自定义分区的唯一目标是将数据均匀地分布到整个集群中，以避免诸如数据倾斜之类的问题。

//如果要使用自定义分区，则应从结构化API定义的数据降级为RDD ，应用自定义分区程序，然后再将RDD转换回DataFrame或Dataset。
//只有真正需要时，才会使用RDD自定义分区，这样就可以利用两方面的优势。
val df = spark.read
  .option("header", "true")
  .option("inferSchema", "true")
  .csv("file:///usr/local/spark/testCode/online-retail-dataset.csv")
val rdd = df.coalesce(10).rdd 
//Spark有两个内置的分区器，你可以在RDD API中调用，
//它们是用于离散值划分的HashPartitioner（基于哈希的分区）以及RangePartitioner（根据数值范围分区），
//这两个分区器分别针对离散值和连续值。Spark的结构化API已经包含了它们，也可以在RDD中使用它们。

//虽然哈希分区和范围分区程序都很有用，但它们是最基本的分区方法。有时，因为数据量很大并且存在严重的数据倾斜，
//（由于某些key对应的value项比其他key对应的value项多很多导致的数据倾斜），你将需要实现一些非常底层的分区方法。
//你希望尽可能多地拆分这些key以提高并行性，并在执行过程中防止OutOfMemoryError错误发生。
import org.apache.spark.HashPartitioner
rdd.map(r => r(6)).take(5).foreach(println) 
val keyedRDD = rdd.keyBy(row => row(6).asInstanceOf[Int].toDouble) 
keyedRDD.partitionBy(new HashPartitioner(10)).take(10)

//一个典型情况是，当且仅当某个key有特定形式时，由于某个key对应的value太多，需要
//把这个key拆分成很多key。例如，数据集中可能对某两个客户的数据处理总是会使分析过程
//崩溃，我们需要对这两个客户数据进行细分，就是说比其他客户ID更细粒度地分解它们。由于
//这两个key倾斜的情况很严重，所以需要特别处理，而其他的key可以被集中到大组中。这虽
//然是一个极端的例子，但你可能会在数据中看到类似的情况:
...
//任何Spark应用程序的第一步都是创建一个SparkSession。在交互模式中，通常已经为你预先创建了，但在应用程序中，你必须自己创建。
//SparkSession是spark 2.x以后才支持的
import org.apache.spark.SparkSession
val spark = SparkSession.builder().appName("Databricks Spark Example")
  .config("spark.sql.warehouse.dir", "/user/hive/warehouse")
  .getOrCreate()

//可能会发现老旧的代码会直接为结构化API创建SparkContext或者SQLContext
//SparkSession中的SparkContext对象代表与Spark集群的连接，
//可以通过它与一些Spark的低级API（如RDD）进行通信，
//在较早的示例和文档中，它通常以变量名sc存储。通过SparkContext，
//你可以创建RDD、累加器和广播变量，并且可以在集群上运行代码。

//大多数情况下，你不需要显式初始化SparkContext，你应该可以通过SparkSession来访问它。
//如果你想要，一般来说你可以通过getOrCreate方法来创建它：
import org.apache.spark.SparkContext
val sc = SparkContext.getOrCreate()

//在新版本中，永远不需要使用SQLContext，尽量避免使用SparkContext

//初始化SparkSession之后就可以执行一些代码了，所有的Spark代码都编译为RDD。
//逻辑指令：Spark代码基本上由转换和动作操作组成，无论是SQL查询，低级的RDD操作还是机器学习算法。
//了解如何使用DataFrame之类的声明式指令并将其转换为物理执行计划，是理解Spark如何在集群上运行的重要一步。

//Spark中的stage代表可以一起执行的任务组，用以在多台机器上执行相同的操作。
//一般来说，Spark会尝试将尽可能多的工作（即作业内部尽可能多的转换操作）加入同一阶段
//但引擎在shuffle操作之后启动新的stage
...
//执行细节：
Spark会自动以流水线的方式一并完成连续的stage和task，例如map操作接着另一个map操作；
另一个执行细节是，对于所有的shuffle操作，Spark会将数据写入持久化存储（如磁盘），并可以在多个作业中重复使用他。

//Spark在写入内存或磁盘之前执行尽可能多的操作，Spark执行的关键优化之一是流水线，在RDD级别或其他级别上执行。
//通过流水线技术，一系列有数据依赖关系的操作，如果不需要任何节点的数据移动，就可以将这些操作合并为一个单独的任务。

//例如，如果你编写一个基于RDD的程序，首先执行map操作，然后是filter操作，
//然后接着另一个map操作，这些操作不需要在节点间移动数据，所以就可以将它们合并为一个单一阶段的任务，
//即读取每个输入记录，然后经由第一个map操作，再执行filter操作，然后如果需要的话再执行map操作。


//////////////////////////////////////////////结构化流处理Structured Streaming
//读取数据
val static = spark.read.json("/data/activity-data/")
val dataSchema = static.schema
//所有静态的结构化API中可用的转换操作都适用于流式DataFrame
//但是一个区别是结构化流处理不允许你在未明确指定它的情况下执行模式推断
//读取一个具有已知可用的数据模式的文件中读取模式dataSchema对象
//并将dataSchema对象从静态DataFrame应用到流式DataFrame
...
//连接操作
//从spark 2.2开始，结构化流处理支持流式DataFrame与静态DataFrame的连接操作
//spark 2.3将增加对链接多个流的支持，可以执行多列连接，并从静态数据中补充数据
...
//滑动窗口(Sliding window)：多时间窗口是可以重叠的
//以运行10分钟为增量的滑动窗口，但我们希望每5分钟获得一次状态
...
//使用水位处理延迟数据
//前面的例子有一个缺陷，我们未指定系统可以接收延迟多久的迟到数据，这意味着spark需要永久存储这些中间数据
//因为我们没有指定一个过期的时间，如果数据迟到超过一定的时间阈值，我们将不会处理它。
//这是用于所有基于事件时间的有状态处理，我们必须指定水位(watermark)，以便确定过期数据，
//这样就可以免于由于长时间不清理过期数据而造成的系统存储压力过大。
...
//如果没有指定接受延迟多晚的数据，spark就会永久将这些数据保留在内存中
//指定watermark可以使其从内存中释放，从而使流处理能够长时间稳定的运行
...
////任意有状态的处理：
//由上述例子，当你有更复杂的窗口概念时情况会有所不同
//而这是任意有状态处理(Arbitrary Stateful Processing)的所长之处。
//执行有状态的处理可能需要执行以下操作：
  // 根据给定键的计数创建窗口
  // 如果在特定事件范围内发生多个特定事件，则发出警报  
  // 在不确定的时间内维护用户会话，保存这些回话以便稍后进行分析

//在一天的最后，如果执行这类处理时，你将需要执行以下两件事情：  
  //映射数据中的组，对每组数据进行操作，并为每个组生成至多一行。这个用例的相关API是mapGroupsWithState。  
  //映射数据中的组，对每组数据进行操作，并为每个组生成一行或多行。这个用例的相关API是flatMapGroupsWithState。  
//当我们说对每组数据进行“操作”时，你可以独立于其他数据组去任意更新该组，
//也就是说你也可以定义不符合翻滚窗口或滑动窗口的任意窗口类型。
//在执行这种处理方式时获得的一个重要好处是可以控制配置状态超时，
//使用时间窗口和watermark，这非常简单：当一个窗口开始前watermark 已经超时，你只需暂停该窗口。
//这不适用于任意有状态处理，因为你是基于用户定义的概念来管理状态，所以你需要适当地设置状态超时。

//输出模式：mapGroupWithState
//类似于用户自定义的聚合函数，它将更新数据集作为输入，然后将其间隙为一个对应值集合的键
//需要给出以下定义：
...
//定义好之后，就可以通过传递相关信息来执行查询。在指定mapGroupWithState时，需要指定是否需要超时给定组的状态，
//它告诉系统如果在一段时间后没有收到更新的状态应该做什么，在这个例子中，希望无限期地维护状态，因此spark不会超时。
//使用update输出模式，以便获得用户活动的更新
